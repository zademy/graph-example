...
const statusStyles = {
  understood: { cls: "st-understood", radius: 5 },
  practicing: { cls: "st-practicing", radius: 5 },
  introduced: { cls: "st-introduced", radius: 3.5 },
  "not yet": { cls: "st-notyet", radius: 3.5 },
};
...
const FILTERS = {
  all: {
    matches: () => true,
    count: () => {
      const lit = concepts.filter(
        (n) => n.status === "understood" || n.status === "practicing",
      ).length;
...
export function createGraphView(doc, { onSelect } = {}) {
  ...
  edgePairs.forEach(([a, b], i) => {
    ...
    line.style.animationDelay = `${-(i % 8) * 0.18}s`;
  ...
  concepts.forEach((n) => {
    ...
    if (n.review) {
      g.appendChild(
        svgEl(doc, "circle", { cx: n.x, cy: n.y, r: 9, class: "review-ring" }),
      );
    }
    ...
    label.style.marginTop = n.important ? "-13px" : "12px";
  ...
  function render(state) {
    ...
    selectionLayer.append(
      svgEl(doc, "circle", {
        ...
        cy: sel.y,
        r: 20,
        class: "solar-glow sun-c",
      }),
      svgEl(doc, "circle", {
        cx: sel.x,
        cy: sel.y,
        r: 17,
        class: "solar-glow sun-a",
      }),
      svgEl(doc, "circle", {
        cx: sel.x,
        cy: sel.y,
        r: 11,
        class: "solar-glow sun-b",
      }),
    );
    ...
    edgeEls.forEach((edge) => {
      const touches =
        edge.dataset.a === state.selected || edge.dataset.b === state.selected;
      const bothVisible =
        passesFilter(edge.dataset.a, state) &&
        passesFilter(edge.dataset.b, state);
    ...
    doc.getElementById("countText").textContent =
      FILTERS[state.filter].count(state);
  ...
  function updatePanel(id) {
    ...
    light.classList.toggle("off", notYet);

    const evidence = doc.getElementById("evidence");
    evidence.replaceChildren();
    rec.evidence.forEach(([text, meta]) =>
      row(evidence, "evidence-item", [
        ["evidence-text", text],
        ["evidence-meta", meta],
      ]),
    );
